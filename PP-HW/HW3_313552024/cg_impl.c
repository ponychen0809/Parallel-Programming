               ...
               double *rnorm)
{
    ...
    //---------------------------------------------------------------------
    // Initialize arrays - parallelizable
    ...
    for (cgit = 1; cgit <= cgitmax; cgit++)
    {
        ...
        // Most time-consuming matrix-vector multiplication part
        #pragma omp parallel for private(sum,k) schedule(static)
        ...
        // and    r = r - alpha*q
        //---------------------------------------------------------------------
        // Combine vector operations to reduce synchronization overhead
        rho = 0.0;
        #pragma omp parallel for reduction(+:rho)
        for (j = 0; j < lastcol - firstcol + 1; j++)
        {
